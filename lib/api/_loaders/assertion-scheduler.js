...
class AssertionScheduler {
  ...
  reschedule() {
    // __writeLog('wait_assertion', 'start');
    setTimeout(() => {
      this.retries++;
      // __writeLog('wait_assertion', 'end');
      this.schedule();
...
module.exports.create = function(instance, opts) {
  __writeLog('assertion_schedular_create', 'start');
  const scheduler = new AssertionScheduler(instance, opts);
  scheduler.verifyCommandArgs();

  return scheduler.start().then(res => {
    __writeLog('assertion_schedular_create', 'end');
  });
};
