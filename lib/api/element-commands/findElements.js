  ...
  async elementFound(response) {
    if (response && Array.isArray(response.value)) {
      response.value = response.value.map(entry => {
        const {getId, findChildElement, findChildElements} = findElementFns(this, entry);

        return Object.assign(entry, {getId, findChildElement, findChildElements});
      });

      const {getElementByIndex, filterVisibleElements, filterByText, filterByCSS} = findElementsFns(this, response.value);

      Object.assign(response.value, {getElementByIndex, filterVisibleElements, filterByText, filterByCSS});
    }
