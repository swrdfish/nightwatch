...
module.exports = class Results {
  ...
  getTestBlock(testName) {
    if (!testName || !this.testblocks[testName]) {
      return this.initialResult;
    }

    return this.testblocks[testName];
  }
  ...
  get export() {
    ...
    suiteResults.results.completedBlocks = this.testblocks;
  ...
  setLastError(err, {incrementTotal, addToErrArray = false} = {}) {
    ...
    if ((!testName || addToErrArray || allowedHooks[testName]) && incrementTotal && detailedLogging) {
  ...
  logCommand(command) {
    const result = this.getTestBlock(this.currentBlockName);
  ...
  setCurrentBlock(testcase) {
    this.currentBlockName = testcase.testName;
    this.testblocks[testcase.testName] = this.createTestCaseResults(testcase);
  }
