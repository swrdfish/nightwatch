...
module.exports = class Results {
  ...
  constructor(tests = [], opts, settings) {
    ...
    const {capabilities = {}, desiredCapabilities = {}, webdriver ={}} = settings;
  ...
  get currentTest() {
    ...
    if (this.testSections[this.currentSectionName]) {
      currentTest.results.commands = this.testSections[this.currentSectionName].commands;
    }
  ...
  get export() {
    ...
    suiteResults.results.status = this.testsPassed() ? Results.TEST_PASS : Results.TEST_FAIL;
    suiteResults.results.seleniumLogs = this.seleniumLogs;
  ...
  setSeleniumLogFile(outputFilePath) {
    this.seleniumLogs = outputFilePath;
  }
