...
class Reporter extends SimplifiedReporter {
  ...
  constructor({settings, tests, suiteRetries, addOpts = {}}) {
    ...
    this.internalCommandPath = path.resolve(__dirname, '../');
    this.assertProxyPath = path.resolve(__dirname, '../api/index.js');
  };
  ...
  setCurrentTest(testcase, context) {
    ...
    this.setCurrentBlock(testcase);
    this.testResults.setCurrentTest(testcase);
  }

  setCurrentBlock(testcase) {
    this.testResults.setCurrentBlock(testcase);
  }
  ...
  shouldLogCommand(node) {
    if (node.parent.context && node.parent.context.isUserDefined) {
      return true;
    }

    if (node && node.stackTrace) {
      const invokationPath = node.stackTrace.split('\n')[1];

      return invokationPath.includes(this.internalCommandPath) 
        && !invokationPath.includes(this.assertProxyPath);
    }
