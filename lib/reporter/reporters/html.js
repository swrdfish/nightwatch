...
class HtmlReporter extends BaseReporter {
  ...
  static hookNames = ['__before_hook', '__after_hook', '__global_beforeEach_hook', '__global_afterEach_hook'];
  static ansiConverter = new AnsiConverter();
  ...
  static computePassedAndFailedCounts(module) {
    ...
    for (const sectionName of Object.keys(module.completedSections)) {
      if (HtmlReporter.hookNames.includes(sectionName)) continue;
  ...
  static aggregateStats(environments) {
    ...
    for (let envName of Object.keys(environments)) {
  ...
  async writeReport() {
    ...
    // TODO: remove this
    fs.writeFileSync(`${__dirname}/report.json`, jsonString);
