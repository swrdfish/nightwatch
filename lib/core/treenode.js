...
class TreeNode {
   ...
   */
  constructor({name, parent, namespace, stackTrace, deferred, isES6Async, compatMode, rejectPromise, insideCallback}) {
    this.__command = null;
    this.__context = null;
    this.__args = null;
  ...
  logCommandResult(result) {
    let reporter = this.context.reporter;

    if (this.parent.context && this.parent.context.reporter) {
      reporter = this.parent.context.reporter;
    } else if (this.client && this.client.context) {
      reporter = this.client.context.reporter;
    } else if (this.options.reporter) {
      reporter = this.options.reporter;
    } else if (this.context.client && this.context.client.reporter) {
      reporter = this.context.client.reporter;
    }

    if (reporter && reporter.logCommandResult) {
      reporter.logCommandResult(this, result);
    }
  }
}
