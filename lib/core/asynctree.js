...
class AsyncTree extends EventEmitter{
  ...
  async runChildNode(node) {
    ...
    AsyncTree.logCommandResult(node, result);
  ...
  static logCommandResult(node, result) {

    let reporter = node.context.reporter;

    if (node.parent.context && node.parent.context.reporter) {
      reporter = node.parent.context.reporter;
    } else if (node.client && node.client.context) {
      reporter = node.client.context.reporter;
    }

    if (reporter && reporter.logCommandResult) {
      reporter.logCommandResult({
        name: node.fullName,
        args: node.args,
        startTime: node.startTime,
        passed: result === undefined || result === null || result.status === 0 || result.passed,
        result
      });
    }
  }
