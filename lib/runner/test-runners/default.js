...
class DefaultRunner {
  ...
  async runTestSuite(modulePath, modules) {
    let currentSuite;
    try {
      currentSuite = await this.createTestSuite({modulePath, modules});
  ...
  promiseFn(resolve, reject) {
    while (this.workers < 8 && this.totalSuites > 0) {
      this.workers++;
      this.totalSuites--;
      const sourcePath = this.modulePathsCopy.shift();

      this.runTestSuite(sourcePath, this.fullPaths).then((result)=> {
        this.workers --;
        if (this.totalSuites === 0 && this.workers === 0) {
          resolve();
        } else {
  ...
  runTests(modules) {
    ...
    this.totalSuites = this.modulePathsCopy.length;
