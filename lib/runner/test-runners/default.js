...
class DefaultRunner {
  ...
  async runTestSuite(modulePath, modules) {
    try {
    __writeLog('default_create_test_suite', 'start');

      this.currentSuite = await this.createTestSuite({modulePath, modules});

    __writeLog('default_create_test_suite', 'end');
    ...
    let possibleErr;
    try {
    __writeLog('default_create_run_testsuit', 'start');

      await this.currentSuite.run();
    __writeLog('default_create_run_testsuit', 'end');
   ...
   */
  runTests(modules) {
    __writeLog('default_runner_run_tests', 'start');

    this.modulePathsCopy = modules.slice(0);
    this.fullPaths = modules;

    return new Promise(this.promiseFn.bind(this)).then(res => {
      __writeLog('default_runner_run_tests', 'end');
  ...
  async runConcurrent(testEnvArray, modules, isTestWorkerEnabled) {
    __writeLog('run_concurrent', 'start');
   ...
   */
  async run(modules) {
    __writeLog('default_runner_run', 'start');
    ...
    __writeLog('default_runner_run', 'end');
