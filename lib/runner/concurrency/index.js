...
class Concurrency extends EventEmitter {
  ...
  runMultiple(envs = [], modules) {
    __writeLog('run_child_processes', 'start');
    
    return this.runChildProcesses(envs, modules)
      .then(_ => {
        __writeLog('run_child_process_phase_2', 'end');

        __writeLog('run_child_processes', 'end');
  ...
  runChildProcesses(envs, modules) {
    __writeLog('run_child_process_phase_1', 'start');
    const availColors = Concurrency.getAvailableColors();
    const args = Concurrency.getChildProcessArgs(envs);

    __writeLog('run_child_process_phase_1', 'end');

    __writeLog('run_child_process_phase_2', 'start');
  ...
  runTestEnvironments(envs, args, availColors) {
    __writeLog('run_test_environments(schedule child processes)', 'start');
    ...
    return Promise.all(envs.map((environment, index) => {
      ...
      __writeLog('run_test_environments(schedule child processes)', 'end');
  ...
  runMultipleTestWorkers(modules, args, availColors) {
    __writeLog('run_multiple_test_workers(schedule child processes)', 'start');
    ...
    return new Promise((resolve, reject) => {
      ...
      __writeLog('run_multiple_test_workers(schedule child processes)', 'end');
