...
const {Logger, Screenshots, Snapshots, alwaysDisplayError, isString, isFunction, isTraceable, SafeJSON} = require('../utils');
...
class TestSuite {
  ...
  async startTestSuite() {
    ...
    return this.createSession()
      .then(data => {
        ...
        this.commandQueue.tree.on('asynctree:command:finished', this.takeSnapshot.bind(this));
        this.commandQueue.tree.on('asynctree:command:finished', this.reporter.logCommandResult.bind(this.reporter));
  ...
  runCurrentTest(testName) {
    ...
    return this.createSession()
      ...
      .then(possibleError => {
        if (this.transport.driverService) {
          this.reporter.testResults.setSeleniumLogFile(this.transport.driverService.getOutputFilePath());
        }
      ...
      return new Promise((resolve) => {
        this.api.saveScreenshot(fileNamePath, (result, err) => {
          if (!err && this.transport.isResultSuccess(result))  {
            ...
            const commands = this.api.currentTest.results.commands || [];
  ...
  takeSnapshot({node, result}) {
    if (this.settings.trace.enabled && isTraceable(node.fullName)) {
      this.snapShotPath = Snapshots.getFileName({
        testSuite: this.api.currentTest.module,
        testCase: this.api.currentTest.name,
      ...
      this.api.saveSnapshot(this.snapShotPath, (result => {
        const commands = this.api.currentTest.results.commands || [];
        const currentCommand = commands[commands.length-1];
        if (currentCommand) {
          this.__snapShot = result;
      ...
      const currentCommand = commands[commands.length-1];
      if (currentCommand) {
        currentCommand.domSnapshot = this.__snapShot;
      }
    }
  }
