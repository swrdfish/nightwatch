...
class TestSuite {
  ...
  runHook(hookName) {
    ...
    if (hookName === 'before' || hookName === 'after') {
      this.client.reporter.setCurrentTest({testName: `${hookName}_hook`}, this.context);
    }
  ...
  runGlobalHook(hookName) {
    ...
    this.client.reporter.setCurrentTest({testName: `global_${hookName}_hook`}, this.context);
