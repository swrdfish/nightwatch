...
const {Logger, Screenshots, alwaysDisplayError, isString, isFunction, SafeJSON} = require('../utils');
...
class TestSuite {
  ...
  runHook(hookName) {
    ...
    if (hookName === 'before' || hookName === 'after') {
      this.client.reporter.setCurrentBlock({testName: `${hookName}_hook`});
    }
  ...
  runGlobalHook(hookName) {
    ...
    this.client.reporter.setCurrentBlock({testName: `global_${hookName}_hook`});
  ...
  async startTestSuite() {
    ...
    return this.createSession()
      .then(data => {
        ...
        if (data && data.capabilities && data.capabilities['safari:deviceUDID']) {
