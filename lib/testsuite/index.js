...
class TestSuite {
  ...
  createReporter() {
    __writeLog('testsuite_create_reporter', 'start');
    ...
    __writeLog('testsuite_create_reporter', 'end');
  }
  ...
  runHook(hookName) {
    __writeLog(`testsuite_run_hook_${hookName}`, 'start');
    ...
    return this.handleRunnable(hookName, () => {
      ...
      __writeLog(`testsuite_run_hook_${hookName}`, 'end');
  ...
  runGlobalHook(hookName) {
    __writeLog(`testsuite_run_globalHook_${hookName}`, 'start');
    ...
    return this.handleRunnable(hookName, async () => {
      ...
      __writeLog(`testsuite_run_globalHook_${hookName}`, 'end');
  ...
  createSession() {
    ...
    __writeLog('testsuite_create_session', 'start');
    ...
    return this.client.createSession({argv, moduleKey, reuseBrowser}).catch(err => {
      ...
      __writeLog('testsuite_create_session', 'end');
  ...
  async startTestSuite() {
    if (!this.client) {
      __writeLog('testsuite_create_client', 'start');

      await this.createClient();

      __writeLog('testsuite_create_client', 'end');

      __writeLog('testsuite_setup_hooks', 'start');

      this.setupHooks();

      __writeLog('testsuite_setup_hooks', 'end');
   ...
   */
  runTestSuite() {
    __writeLog('testsuite_run_test_suite', 'start');
    ...
    return this.startTestSuite()
      ...
      .then(result => {
        ...
        __writeLog(`testsuite_run_next_test_case`, 'start');

        return this.runNextTestCase().then(res => {
          __writeLog(`testsuite_run_next_test_case`, 'end');
      ...
      .then((errorOrFailures) => this.onTestSuiteFinished(errorOrFailures)).then(res => {
        __writeLog('testsuite_run_test_suite', 'end');
  ...
  runCurrentTest(testName) {
    ...
    __writeLog('testsuit_run_current_test', 'start');

    
    const {reporter, context, settings} = this;
    
    const modulePath = `${context.modulePath}`;

    __writeLog(`test_case_name::${modulePath}::${testName}`, 'start');
    ...
    return this.createSession()
      ...
      .then(res => {
        __writeLog(`test_case_name::${modulePath}::${testName}`, 'end');
        __writeLog('testsuit_run_current_test', 'end');
  ...
  testCaseFinished() {
    __writeLog('testsuit_test_case_finished', 'start');
  ...
  async executeRunnable(name, fn) {
    __writeLog('testsuite_execute_runnable', 'start');
    ...
    try {
      ...
      __writeLog('testsuite_execute_runnable', 'end');
