...
class Transport extends BaseTransport {
  ...
  async createDriverService({options, moduleKey, reuseBrowser = false}) {
    try {
      ...
      await this.driverService.setOutputFile(reuseBrowser ? 'test' : moduleKey).init(options);
      // }
  ...
  async getDriver({options, reuseBrowser = false}) {
    const driver = await this.createDriver({options});
  ...
  async createSession({argv, moduleKey, reuseBrowser = false}) {
    ...
    const portStr = 'auto-generated port';
