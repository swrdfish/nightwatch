...
const {Logger, isObject, IosSessionNotCreatedError, AndroidConnectionError} = require('../../utils');
...
class Transport extends BaseTransport {
  ...
  async getDriver({options, reuseBrowser = false}) {
    ...
    __writeLog('getDriver_shouldReuseDriver', 'start');
    const value  = await this.shouldReuseDriver(reuseBrowser);
    __writeLog('getDriver_shouldReuseDriver', 'end');

    if (value) {
      return Transport.driver;
    }

    __writeLog('getDriver_createDriver', 'start');

    Transport.driver = await this.createDriver({options});
    __writeLog('getDriver_createDriver', 'end');
  ...
  async createSession({argv, moduleKey, reuseBrowser = false}) {
    ...
    if (start_process) {
      ...
      __writeLog('create_driver_service', 'start');
      await this.createDriverService({options, moduleKey, reuseBrowser});
      __writeLog('create_driver_service', 'end');
    } else {
      this.showConnectSpinner(`Connecting to ${host} on ${portStr}...\n`);
    }

    try {
      __writeLog('create_session_getDriver', 'start');
      this.driver = await this.getDriver({options, reuseBrowser});
      __writeLog('create_session_getDriver', 'end');

    __writeLog('transport_createsession_after_getDriver', 'start');
      ...
      __writeLog('transport_createsession_after_getDriver', 'end');

      __writeLog('transport_create_session', 'end');
  ...
  handleConnectError(err) {
    ...
    if (!err.detailedErr && this.driverService) {
      ...
      if (err.name === 'SessionNotCreatedError' && this.api.isSafari() && this.api.isIOS()) {
