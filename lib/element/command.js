...
class ElementCommand extends EventEmitter {
  ...
  async findElement({returnSingleElement = true, cacheElementId = true, id, transportAction = 'locateMultipleElements'} = {}) {
    ...
    try {
      ...
      return await this.elementLocator.findElement({element, commandName, returnSingleElement, cacheElementId, id, transportAction});
    } catch (err) {
  ...
  setArguments(requireValidation) {
    ...
    if (!this.selectorPassed) {
      this.args.unshift('*');
    }
    
    this.__selector = this.args.shift();

    if (this.args[0] && this.args[0].name == 'parentId') {
      this.__parentId = this.args.shift()();
    }
  ...
  setOptionsFromSelector() {
    ...
    if (Utils.isValidXpath(this.selector)) {
      this.setStrategy(Strategies.XPATH);
    }
